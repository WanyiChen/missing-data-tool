...
const ThirdQuestion: React.FC<ThirdQuestionProps> = ({
  ...
  const columnNames: string[] = previewRows[0]?.map((col: any, i: number) => String(col)) || [];
  const filteredColumns = columnNames.filter(name => name.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleFeatureSelect = (name: string) => {
    ...
    if (targetType === null) {
      ...
      const isCategorical = colValues.some(val => typeof val === 'string' && val.trim() !== '' && isNaN(Number(val)));
      const isNumerical = colValues.every(val => val === undefined || val === null || val === '' || (!isNaN(Number(val)) && val !== ''));
      if (isCategorical) {
      ...
      <div className="w-full max-w-4xl px-4 py-8">
        ...
        <div className="mb-6 mt-8">
          <label className="block text-lg font-medium mb-2">
            What is your target feature? If you are training machine learning models, your target feature is the feature you are trying to predict.
          </label>
          <div className="relative w-full max-w-xs" ref={dropdownRef}>
            ...
            >
              {targetFeature || <span className="text-gray-400">Type to search</span>}
            ...
            {dropdownOpen && (
              <div className="absolute z-10 mt-1 w-full bg-white border rounded-lg shadow-lg max-h-60 overflow-y-auto">
                ...
                {filteredColumns.length === 0 ? (
                  <div className="px-4 py-2 text-gray-400 text-sm">No features found</div>
                ) : (
                  filteredColumns.map((name, i) => (
                    <div
                      key={i}
                      className={`px-4 py-2 cursor-pointer hover:bg-gray-100 text-sm ${targetFeature === name ? 'bg-gray-200 font-semibold' : ''}`}
              ...
              <thead>
                <tr>
                  {columnNames.map((col, i) => (
                    <th key={i} className="px-3 py-2 border-b font-semibold text-xs text-gray-700 whitespace-nowrap bg-gray-50">{col}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
